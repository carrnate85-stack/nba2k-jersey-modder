...
from .font_recolor import (_average_rgb, _blend_rgb, _clamp, _font_alpha_edge_distances, _font_distance_fill_mixes, _font_fill_region_mixes, _font_outline_fill_color_centers, _percentile, _recolor_font_image, _rgb_distance, _smoothstep, _thicken_font_outline_mixes)
...
class JerseyModderApp(tk.Tk):
    ...
    def _trim_path_lab_send_to_generator(self, payload: dict) -> dict:
        ...
        garment_value = str(payload.get("garment") or "Shorts").strip()
        garment = "Jersey" if garment_value.casefold() == "jersey" else "Shorts"
        template_name = str(payload.get("templateName") or "").strip()
        output_dir = (
            Path(tempfile.gettempdir())
            / "nba2k_jersey_modder"
            / "trim_paths"
            / safe_filename(garment).replace(" ", "_").lower()
            / safe_filename(template_name or "default").replace(" ", "_").lower()
        )
        ...
        for index, item in enumerate(raw_layers, start=1):
            ...
            try:
                with Image.open(BytesIO(image_data)) as opened:
                    trim_image = opened.convert("RGBA")
                visible_bounds = trim_image.getchannel("A").getbbox()
            except (OSError, ValueError):
                continue
            if visible_bounds is None:
                continue
            cropped = trim_image.crop(visible_bounds)
            ...
            received.append(
                TrimPathLayer(
                    ...
                    default_x=visible_bounds[0],
                    default_y=visible_bounds[1],
                    default_width=cropped.width,
                    default_height=cropped.height,
                )
            )
        if not received:
        ...
        self.generator_trim_path_layers = [
            ...
            if not self._trim_path_layer_matches_scope(
                layer,
                garment=garment,
                template_name=template_name,
            )
        ] + received
        self.generator_garment_var.set(garment)
        if garment == "Shorts" and template_name in SHORTS_TEMPLATE_OPTIONS:
            self.generator_shorts_template_var.set(template_name)
        elif garment == "Jersey" and template_name in JERSEY_CUT_OPTIONS:
            self.generator_jersey_cut_var.set(template_name)
        self._sync_generator_template_controls(refresh_preview=False)
        self._sync_generator_trim_path_layer_order()
        self._refresh_generator_trim_path_list()
    ...
    def open_game_font_browser(self) -> None:
        ...
        self.game_font_thumbnail_lock = threading.Lock()
        if not hasattr(self, "game_font_cache_all_active"):
            self.game_font_cache_all_active = False
        if (
            not hasattr(self, "game_font_cache_stop_event")
            or not self.game_font_cache_all_active
        ):
            self.game_font_cache_stop_event = threading.Event()
        self.game_font_cache_progress_var = tk.DoubleVar(value=0)
        ...
        self.game_font_cache_all_button = ttk.Button(
            header,
            text="Cache All Previews",
            command=self.cache_all_game_font_previews,
        )
        self.game_font_cache_all_button.pack(side=tk.RIGHT, padx=(0, 8))
        self.game_font_stop_cache_button = ttk.Button(
            header,
            text="Stop",
            command=self.stop_game_font_preview_cache,
        )
        self.game_font_stop_cache_button.pack(side=tk.RIGHT, padx=(0, 8))
        if self.game_font_cache_all_active:
            self.game_font_cache_all_button.state(["disabled"])
            self.game_font_stop_cache_button.state(["!disabled"])
        else:
            self.game_font_stop_cache_button.state(["disabled"])
        self.game_font_clear_cache_button = ttk.Button(
            header,
            text="Clear Preview Cache",
            command=self.clear_game_font_preview_cache,
        )
        self.game_font_clear_cache_button.pack(side=tk.RIGHT, padx=(0, 8))
        if self.game_font_cache_all_active:
        ...
        progress_frame = ttk.Frame(browser, padding=(10, 0, 10, 10))
        progress_frame.grid(row=4, column=0, columnspan=2, sticky="ew")
        self.game_font_cache_progress = ttk.Progressbar(
            progress_frame,
            variable=self.game_font_cache_progress_var,
            mode="determinate",
        )
        self.game_font_cache_progress.pack(fill=tk.X, expand=True)
    ...
    def _close_game_font_browser(self) -> None:
        self.game_font_cache_stop_event.set()
    ...
    def preview_selected_game_font(self) -> None:
        ...
        def worker() -> None:
            try:
                preview_path, preview_label, _was_cached = (
                    self._ensure_game_font_thumbnail(
                        entry,
                        keep_working_iff=True,
                    )
                )
            except Exception as exc:  # noqa: BLE001 - background GUI boundary.
                self.after(
    ...
    def _ensure_game_font_thumbnail(
        ...
        entry: ManifestEntry,
        *,
        keep_working_iff: bool,
    ) -> tuple[Path, str, bool]:
        from PIL import Image

        with self.game_font_thumbnail_lock:
            ...
            if metadata is not None:
                ...
                if keep_working_iff:
                    ...
                    self._prune_game_font_iff_cache()
                return preview_path, label, True
            ...
            return preview_path, label, False

    def _prepare_game_font_cache(self) -> None:
        self._migrate_legacy_game_font_thumbnail_cache()
    ...
    def _migrate_legacy_game_font_thumbnail_cache(self) -> None:
        legacy_root = Path(
        ...
        target_cache = self._game_font_thumbnail_cache_dir()
        if not legacy_cache.is_dir() or legacy_cache.resolve() == target_cache.resolve():
            return
        target_cache.mkdir(parents=True, exist_ok=True)
        for pattern in ("*.webp", "*.json"):
            for source in legacy_cache.glob(pattern):
    ...
    def clear_game_font_preview_cache(self) -> None:
        if self.game_font_cache_all_active:
            self.game_font_browser_status_var.set(
                "Stop preview caching before clearing the cache."
            )
            return
        cache_dir = self._game_font_thumbnail_cache_dir()
        removed = 0
    ...
    def cache_all_game_font_previews(self) -> None:
        if self.game_font_cache_all_active or not self.game_font_entries:
            return
        ...
        self.game_font_stop_cache_button.state(["!disabled"])
        self.game_font_clear_cache_button.state(["disabled"])
        entries = list(self.game_font_entries)
        self.game_font_cache_progress.configure(maximum=max(1, len(entries)))
        self.game_font_cache_progress_var.set(0)
        self.game_font_browser_status_var.set(
            f"Caching previews: 0 of {len(entries):,}..."
        )
        ...
        def worker() -> None:
            ...
            errors = 0
            for entry in entries:
                if self.game_font_cache_stop_event.is_set():
                    break
                try:
                    _path, _label, was_cached = self._ensure_game_font_thumbnail(
                        entry,
                        keep_working_iff=False,
                    )
                    if not was_cached:
                        created += 1
                except Exception:  # noqa: BLE001 - continue building the catalog.
                    errors += 1
                completed += 1
                if completed == 1 or completed % 5 == 0:
                    self.after(
                        0,
                        lambda done=completed, new=created, failed=errors: (
                            self._update_game_font_cache_progress(
                                done,
                                len(entries),
                                new,
                                failed,
            ...
            stopped = self.game_font_cache_stop_event.is_set()
            self.after(
                0,
                lambda: self._finish_game_font_cache_all(
                    completed,
                    len(entries),
                    created,
                    errors,
    ...
    def stop_game_font_preview_cache(self) -> None:
        self.game_font_cache_stop_event.set()
        browser = getattr(self, "game_font_browser", None)
        if browser is not None and browser.winfo_exists():
            self.game_font_browser_status_var.set(
                "Stopping after the current font finishes..."
            )

    def _update_game_font_cache_progress(
        self,
        completed: int,
        total: int,
        created: int,
        ...
        self.game_font_browser_status_var.set(
            f"Caching previews: {completed:,} of {total:,} | "
            f"{created:,} new | {errors:,} skipped"
        )

    def _finish_game_font_cache_all(
        self,
        completed: int,
        total: int,
        created: int,
        ...
        if stopped:
            self.game_font_browser_status_var.set(
                ...
                "Cache All Previews will resume the missing entries."
            )
        else:
            self.game_font_browser_status_var.set(
                f"Preview cache complete: {total:,} fonts | "
                f"{created:,} new | {errors:,} skipped."
            )
    ...
    def clear_generator_trim_paths(self) -> None:
        self.generator_trim_path_layers = [
            layer
            for layer in self.generator_trim_path_layers
            if not self._trim_path_layer_matches_scope(layer)
        ]
        self._sync_generator_trim_path_layer_order()
        self._refresh_generator_trim_path_list()
        self._schedule_generator_preview_refresh()

    def _current_generator_trim_path_scope(self) -> tuple[str, str]:
    ...
    def _trim_path_layer_matches_scope(
        self,
        layer: TrimPathLayer,
        *,
        ...
        if garment is None or template_name is None:
            current_garment, current_template = self._current_generator_trim_path_scope()
            garment = garment or current_garment
            template_name = template_name if template_name is not None else current_template
        if layer.garment.casefold() != garment.casefold():
            return False
        return not layer.template_name or layer.template_name.casefold() == template_name.casefold()

    def _active_generator_trim_path_layers(self) -> list[tuple[int, TrimPathLayer]]:
        return [
            (index, layer)
            for index, layer in enumerate(self.generator_trim_path_layers)
            if self._trim_path_layer_matches_scope(layer)
        ]

    def _active_generator_trim_path_layer(
        self,
        active_index: int,
    ) -> tuple[int, TrimPathLayer] | None:
        active = self._active_generator_trim_path_layers()
        if 0 <= active_index < len(active):
    ...
    def _sync_generator_trim_path_layer_order(self) -> None:
        trim_keys = [
            f"trim_path:{index}"
            for index, _layer in enumerate(self._active_generator_trim_path_layers())
        ]
        ordered = [
            key
            for key in self.web_editor_layer_order
            if not key.startswith("trim_path:") or key in trim_keys
        ]
        missing = [key for key in trim_keys if key not in ordered]
        self.web_editor_layer_order = missing + ordered
    ...
    def _refresh_generator_trim_path_list(self) -> None:
        ...
        for index, layer in self._active_generator_trim_path_layers():
            ...
            overlays.append(
                {
                    ...
                    "clipBox": None,
                    "guideBox": None,
                    "excludeBoxes": waistband_boxes,
                    "canCleanup": False,
                    "cleanup": self._web_editor_cleanup_payload(key),
                    "canReorder": True,
    ...
    def _web_editor_order_layers(self, overlays: list[dict]) -> list[dict]:
        ...
        for index, item in enumerate(ordered, start=1):
            if (
                item["key"].startswith("logo:")
                or item["key"].startswith("trim_path:")
                or item["key"] == "fabric_overlay"
            ):
    ...
    def _active_dynamic_layer_order(self, current_keys: list[str] | None = None) -> tuple[str, ...]:
        ...
        if current_keys is None:
            current_keys = [
                f"trim_path:{index}"
                for index, _entry in enumerate(self._active_generator_trim_path_layers())
            ]
            current_keys.extend(
    ...
    def _web_editor_image(self, key: str) -> tuple[bytes, str]:
        if key.startswith("trim_path:"):
            try:
                active_index = int(key.split(":", 1)[1])
            except ValueError:
                active_index = -1
    ...
    def _web_editor_update(self, payload: dict) -> dict | None:
        ...
        if key.startswith("trim_path:"):
            ...
            if entry is None:
                return
            stored_index, layer = entry
            self.generator_trim_path_layers[stored_index] = replace(
                layer,
                x=max(-8192, min(8192, round(x))),
                y=max(-8192, min(8192, round(y))),
                width=max(1, min(8192, round(width))),
                height=max(1, min(8192, round(height))),
                rotation_degrees=max(-360.0, min(360.0, rotation)),
            )
            self._schedule_generator_preview_refresh()
            return
    ...
    def _web_editor_reset(self) -> None:
        ...
        for stored_index, layer in self._active_generator_trim_path_layers():
            self.generator_trim_path_layers[stored_index] = replace(
                layer,
                x=layer.default_x,
                y=layer.default_y,
                width=layer.default_width,
                height=layer.default_height,
    ...
    def open_blender_preview(self) -> None:
        ...
        preview_mode = (
            "with the normal map"
            if self._blender_preview_normal_node_strength()
            else "color only"
        )
        self.texture_creator_status.configure(
            text=(
    ...
    def _current_blender_preview_scope(self) -> tuple[str, str]:
        garment = self.texture_creator_garment_var.get()
        template_name = (
            self.texture_creator_shorts_template_var.get()
            if garment == "Shorts"
            else self.texture_creator_jersey_cut_var.get()
        )
        return garment, template_name
                ...
                "trimPathLayers": [
                    {
                        ...
                        "templateName": layer.template_name,
                        "x": layer.x,
                        "y": layer.y,
                        "width": layer.width,
                        "height": layer.height,
                        "rotationDegrees": layer.rotation_degrees,
                        "defaultX": layer.default_x,
                        "defaultY": layer.default_y,
                        "defaultWidth": layer.default_width,
                        "defaultHeight": layer.default_height,
                    }
                    for layer in self.generator_trim_path_layers
                ],
                "trimPlacements": {
    ...
    def _apply_project_payload(self, payload: dict) -> list[str]:
        ...
        if isinstance(trim_path_layers, list):
            for index, item in enumerate(trim_path_layers, start=1):
                ...
                if path is None:
                    continue
                layer_garment = str(item.get("garment") or "Shorts")
                fallback_template = (
                    self.generator_shorts_template_var.get()
                    if layer_garment == "Shorts"
                    else self.generator_jersey_cut_var.get()
                )
                self.generator_trim_path_layers.append(
                    TrimPathLayer(
                        ...
                        path=path,
                        garment=layer_garment,
                        template_name=str(
                            item.get("templateName") or fallback_template
                        ),
                        x=self._project_int(item.get("x"), 0, -8192, 8192),
                        y=self._project_int(item.get("y"), 0, -8192, 8192),
                        width=self._project_int(item.get("width"), 2048, 1, 8192),
                        height=self._project_int(item.get("height"), 2048, 1, 8192),
                        rotation_degrees=self._project_float(
                            item.get("rotationDegrees"), 0.0, -360.0, 360.0
                        ),
                        default_x=self._project_int(
                            item.get("defaultX", item.get("x")), 0, -8192, 8192
                        ),
                        default_y=self._project_int(
                            item.get("defaultY", item.get("y")), 0, -8192, 8192
                        ),
                        default_width=self._project_int(
                            item.get("defaultWidth", item.get("width")),
                            2048,
                            1,
                            8192,
                        ),
                        default_height=self._project_int(
                            item.get("defaultHeight", item.get("height")),
                            2048,
                            1,
                            8192,
                        ),
    ...
    def _generator_inputs(
        ...
        *,
        ...
        garment = garment or self.generator_garment_var.get()
        ...
        return GeneratorInputs(
            ...
            trim_path_layers=tuple(
                layer
                for layer in self.generator_trim_path_layers
                if self._trim_path_layer_matches_scope(
                    layer,
                    garment=garment,
                    template_name=template_name,
                )
